...
const unsigned char login[] = "MomoLeHero";
const unsigned char password[] = "Mimolette";
...
int	test_mdp()
{
	for (int j = 0; j < 32; j++)
	{
		if (test_login[j] != login[j] || test_password[j] != password[j])
			return 0;
	}
}
...
ISR(PCINT2_vect)
{
	...
	else if (state == 2)
	{
		if (rchar == '\r')
		{
			...
			if (test_mdp() == 1)
			{
				uart_prinstr("Welcome!\n\r");
				state = 3;
			}
			else
			{
				uart_prinstr("Wrong password!\n\r");
				state = 0;
			}
			test_login[0] = 0;
			test_password[0] = 0;
