...
/*   Updated: 2023/03/22 12:28:47 by lgiband          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	for(;;) {
       ...
       // uint8_t val = adc_read();
		...
		if (state == 1)
			...
			ADMUX |= (1 << MUX1);
	   	buffer[state] = ADCL;
		buffer[state] = (buffer[state]) | (((uint16_t)ADCH) << 8);
		if (state == 2) {
			//usart_dump16ln("", buffer[1], ", ");
			//usart_dump16ln("", buffer[2], ", ");
    		//usart_dump16ln("", buffer[0], "\r\n");
			usart_int16ln(buffer[1]);
			usart_printstr(", ");
			usart_int16ln(buffer[2]);
			usart_printstr(", ");
			usart_int16ln(buffer[0]);
			usart_printstr("\r\n");
		}
        _delay_ms(20);
		state = (state + 1) % 3;
